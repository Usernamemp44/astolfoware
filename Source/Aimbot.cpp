...
void CAimbot::Run(CBaseEntity* pLocal, CUserCmd* pCommand)
{
	...
	if (!Utils::IsKeyPressed(key.value))
		return;
...
#pragma endregion
	...
	if (autoscope.value && !pLocal->IsScoped() && pLocal->szGetClass() == "Sniper")
	{
...
int CAimbot::GetBestTarget(CBaseEntity* pLocal, CUserCmd* pCommand)
{
	...
	for (int i = 1; i <= gInts.Engine->GetMaxClients(); i++)
	{
		...
		float distance = Utils::flGetDistance(vEntity, pLocal->GetEyePosition());
...
int CAimbot::GetBestHitbox(CBaseEntity* pLocal, CBaseEntity* pEntity)
{
	...
	if (!Utils::IsVisible(pLocal, pEntity, pLocal->GetEyePosition(), pEntity->GetHitboxPosition(iBestHitbox)))
		return -1;
