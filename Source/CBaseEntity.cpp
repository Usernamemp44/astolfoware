...
bool CBaseEntity::IsCritBoostedNoMini() {

	return (GetCond() & TFCond_Kritzkrieged ||
		GetCond() & TFCondEx_CritCanteen ||
		GetCond() & TFCondEx_CritOnFirstBlood ||
		GetCond() & TFCondEx_CritOnWin ||
		GetCond() & TFCondEx_CritOnKill ||
		GetCond() & TFCondEx_CritDemoCharge ||
		GetCond() & TFCondEx_CritOnFlagCapture ||
		GetCond() & TFCondEx_HalloweenCritCandy ||
		GetCond() & TFCondEx_PyroCrits);
}
...
Vector CBaseEntity::GetHitboxPosition(int iHitbox)
{
	...
	Utils::VectorTransform(center, matrix[box->bone], vHitbox);
