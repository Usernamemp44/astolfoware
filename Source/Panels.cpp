...
void __fastcall Hooked_PaintTraverse(void* pPanels, void* edx, unsigned int vguiPanel, bool forceRepaint, bool allowForce)
{
	try
	{
		...
		{
			crosshair->SetValue((pLocal->GetCond() & tf_cond::TFCond_Zoomed) ? 0 : 1);
			return;
		}
		...
		static WindowStyle* style = new DefaultStyle();
		if (vguiFocusOverlayPanel == vguiPanel )
		{
			...
			if (!gInts.Engine->IsInGame() && !gMenu.enabled)
				style->WaterMark("Astolfoware", 20, 20);
