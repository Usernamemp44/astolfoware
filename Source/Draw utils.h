...
public:
	...
	inline std::uint8_t& operator[](int i)
	{
		return rgba[i];
	}
	...
	SColor(std::uint8_t red, std::uint8_t green, std::uint8_t blue, std::uint8_t alpha = 255)
	{
		rgba[0] = red, rgba[1] = green, rgba[2] = blue, rgba[3] = alpha;
	}
	SColor(std::uint8_t bright, std::uint8_t alpha = 255)
	{
