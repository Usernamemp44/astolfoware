...
//how tf do you mess up backtrack.....
namespace backtrack {
	...
	void backtrack::backtrack_calc() {
		...
		new_view_direction = Utils::AngleVector(view_direction);
		...
		for (int i = 0; i < gInts.Engine->GetMaxClients(); i++) {
			...
			float field_of_view_distance = Utils::DistPointToLine(entity->GetEyePosition(), g.local->GetEyePosition(), new_view_direction);
		...
		if (best_target_index != -1) {
			for (unsigned int i = 0; i < ticks[best_target_index].size(); i++) {
				float field_of_view_distance = Utils::DistPointToLine(ticks[best_target_index].at(i).head_position, g.local->GetEyePosition(), new_view_direction);
				if (field_of_view_distance < best_field_of_view) {
					best_field_of_view = field_of_view_distance;
