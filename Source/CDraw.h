...
public:
	TextureHolder() {}
	TextureHolder(const std::uint8_t* pRawRGBAData, int W, int H)
	{
	...
	const std::uint8_t* GetRGBA() const
	{
		return rawData;
	}
...
public:
	...
	//void DrawString2(const size_t& font_idx, int x, int y, const Color_t& clr, const EStringAlign& align, const wchar_t* str, SColor color, const wchar_t* wcText, HFont font);

	std::uint8_t GetESPHeight();
