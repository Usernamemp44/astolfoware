...
void CAutoSticky::Run(CBaseEntity* g_pLocalPlayer, CUserCmd* g_pUserCmd)
{
	...
	auto IsLocalGrenade = [&](CBaseEntity* entity) 
	{
		...
		auto entity_handle = (CBaseEntity*)gInts.EntList->GetClientEntityFromHandle(get_thrower);
	...
	{
		CBaseEntity* first_entity = reinterpret_cast<CBaseEntity*>(gInts.EntList->GetClientEntity(i));
		if (!first_entity) {
			continue;
		}
		...
		if (IsLocalGrenade(first_entity)) {
			for (int player = 0; player < gInts.Engine->GetMaxClients(); player++) {
				...
				CBaseEntity* second_entity = reinterpret_cast<CBaseEntity*>(gInts.EntList->GetClientEntity(player));
